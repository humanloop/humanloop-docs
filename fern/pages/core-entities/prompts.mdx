...
- all the paramaters that define how the model should behave such as `temperature`, `max_tokens`, `top_p` etc.
- any tools available to the model

Inputs are defined in the template through the double-curly bracket syntax e.g. `{{topic}}` and the value to variable will need to be supplied when you call the Prompt to create a generation.
...
Your can also use Prompts without proxying all requests through Humanloop.

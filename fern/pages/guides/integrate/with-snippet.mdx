...
<Warning title="Changing a snippet value can change a model configs behaviour">
  Since the values for a Snippet are saved on the tool, not the model config,
  changing the values (or keys) defined in your Snippet tools could affect the
  relative model config behaviour. This could be exactly what you intend,
  however caution should still be used make sure the changes are expected.
</Warning>

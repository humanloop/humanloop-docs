...
<img src="../../assets/images/500686e-image.png" />
...
### Ensure the environment then shows the selected deployment. 
...
- You have already deployed either a chat or completion model config - if not, please follow the steps in either the [Generate chat responses](https://dash.readme.com/project/humanloop/v4.0/docs/chat-using-the-sdk) or [Generate completions](https://dash.readme.com/project/humanloop/v4.0/docs/completion-using-the-sdk) guides. 
- You have multiple environments, with a model config deployed in a non-default environment. See the [Deploying to an environment](https://docs.humanloop.com/docs/deploy-to-an-environment#deploying-to-an-environment) section above. 
...
1. You have multiple environments - if not first go through the [Create an environment](https://docs.humanloop.com/docs/deploy-to-an-environment#create-an-environment) section.
...
### Click on the dropdown menu of an environment that is not already the default environment.

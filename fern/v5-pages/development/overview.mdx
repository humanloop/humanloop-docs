...
You can create a Prompt explicitly [in the Prompt Editor](/docs/v5/development/guides/create-prompt) or [via the API](/docs/v5/reference/api-reference/prompts/create).
...
Our `.prompt` file format is a serialized version of a model config that is designed to be human-readable and suitable for checking into your version control systems alongside your code. See the [.prompt files reference](../reference/prompt-file) reference for more details.
...
For PII detection, you can set up [Guardrails](/docs/v5/observability/alerts-and-guardrails) to detect and prevent the generation of sensitive information.

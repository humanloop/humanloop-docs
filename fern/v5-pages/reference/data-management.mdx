---
subtitle: Data management and encryption
description: Learn about Humanloop's commitment to data management and encryption.
...
Humanloop follows the best practices for data management and encryption. All data in transit is be secured with TSL/SSL and all data at rest is encrypted using the AES-256 algorithm. All encryption keys are managed using AWS Key Management Service (KMS) as part of the VPC definition.

### Infrastructure

All sensitive data is encrypted in transit. For Self Hosted Cloud (VPC) environments, network traffic is also encrypted in transit and at rest in order to meet HIPAA requirements. Sensitive application data is only every processed within the ECS cluster and stored in Aurora. To request a network infrastructure diagram or more information, please contact [privacy@humanloop.com](mailto:privacy@humanloop.com).

### Learn More

For more information on about how Humanloop processes user data, visit our Security & Compliance page.

### Data Storage, Retention, and Recovery

All platform data is stored in a primary database server with multi-availability zone replication. Platform data is retained indefinitely and backed up daily in a secure and encrypted manner until a request is made from the contractual owners of that data to remove it and in accordance to GDPR guidelines.

Humanloop’s Terms of Service defines the contractual owner of the user data and data derived from the user data. A semi-automated disaster recovery process is in place to restore the database to a specified point in time backup as and when required.

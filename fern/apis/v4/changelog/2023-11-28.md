...
We've added support for managing [evaluations](/docs/evaluate-your-model) outside of Humanloop in your own code.
...
For example, you may have implemented an evaluator that uses your own custom model, or has to interact with multiple systems. In which case, it can be difficult to define these as a simple code or [LLM evaluator](/docs/use-llms-to-evaluate-logs) within your Humanloop project.
...
Below is a code snippet showing how you can use the latest version of the Python SDK to log an evaluation run to a Humanloop project. For a full explanation, see our [guide](/docs/self-hosted-evaluations) on self-hosted evaluations.
...
We've updated the response models of all of our [/chat](/api-reference/humanloop-api/chats/create) API endpoints to include an output message object. 
